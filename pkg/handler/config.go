...
func (h configHandler) GetLog() *zap.Logger {
	return h.log
}
	...
	for _, u := range h.cfg.Users {
		...
		if len(u.CustomAttrs) > 0 {
			for key, attr := range u.CustomAttrs {
				...
				default:
					h.log.Info("Unable to map custom attribute", zap.String("key", key),
						zap.Any("value", attr))
				}
			}
		}
...
func (h configHandler) getGroupMemberIDs(gid int) []string {
	...
	for _, g := range h.cfg.Groups {
		if gid == g.GIDNumber {
			for _, includegroupid := range g.IncludeGroups {
				if includegroupid == gid {
					h.log.Info("Ignoring myself as included group", zap.Int("groupid", includegroupid))
				} else {
